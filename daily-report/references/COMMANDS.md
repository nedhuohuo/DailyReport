...
| 命令 | 功能 | 默认行为 | 参数 |
|------|------|---------|------|
| `/dr_init` | 交互式初始化 | 配置工作区、扫描注册仓库、展示用户信息 | — |
| `/dr_daily` | 生成日报 | 标准详细度 + diff 分析 | `--level=`、`--date=` |
| `/dr_weekly` | 生成周报 | 粗略详细度 + 日报汇总 | `--level=`、`--diff` |
| `/dr_monthly` | 生成月报 | 粗略详细度 + 周报日报汇总 | `--level=`、`--diff` |
| `/dr_status` | 查看状态 | 展示配置和仓库信息 | — |
...
1. **加载配置**：读取 `config.json`，不存在则提示执行 `/dr_init`
...
```bash
SKILL_DIR="$(dirname "$(readlink -f ~/.qoder/skills/daily-report/SKILL.md)")"
```
